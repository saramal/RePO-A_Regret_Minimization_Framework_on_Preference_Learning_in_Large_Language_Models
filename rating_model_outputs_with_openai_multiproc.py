...
def load_processed_prompt_ids_from_parts(base_output_path: str) -> set[str]:
    """
    base_output_path: final merged file path (e.g. "eval_combined.jsonl")
    - this function finds all part*.jsonl files under base_output_path and returns the union of prompt_ids in them.
    """
...
def merge_output_parts(part_paths: list[str], final_output_path: str):
    """
    merge part jsonl files sequentially into a single jsonl file.
    """
...
def run_multiprocess_eval(
    ...
    """
    - divide input_path into num_workers chunks and process in parallel
    - each worker writes to output_path + f".part{rank:02d}.jsonl"
    - finally merge part files into output_path
    """
...
def retry_with_backoff_and_rate_limit_split(
    ...
    - UnknownError: immediate failure
    """
...
def evaluate_jsonl_to_combined_ratings(
    ...
    start_idx, end_idx: line index range to process for multiprocessing (0-based, [start_idx, end_idx))
    """
