...
deepspeed --module openrlhf.cli.train_RePO \
   --save_path ./checkpoint/Qwen2.5-Math-7B-RePO-step \
   --save_steps -1 \
   --logging_steps 1 \
   --eval_steps -1 \
   --train_batch_size 32 \
   ...
   --zero_stage 2 \
   --learning_rate 5e-7 \
   --dataset "./RePO_datasets/stepDPO_step/RePO_train.jsonl, ./RePO_datasets/stepDPO_step/RePO_test.jsonl" \
   --prompt_key prompt \
   --chosen_key chosen \
   ...
   --sft_loss \
   --use_wandb \
   --wandb_run_name RePO_qwen7B_step \
