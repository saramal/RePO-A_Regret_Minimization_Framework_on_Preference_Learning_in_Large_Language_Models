...
#SBATCH --job-name=RePO_topk_sweep
...
experiments=(
    "checkpoint/sft_Qwen/Qwen3-8B-Base_lora_128_metamathqa_0920T03:38_merged 128 1 Qwen3-8B-Base 0.1 5e-7 2"
    "checkpoint/sft_Qwen/Qwen3-8B-Base_lora_128_metamathqa_0920T03:38_merged 128 1 Qwen3-8B-Base 0.1 5e-7 5"
    "checkpoint/sft_Qwen/Qwen3-8B-Base_lora_128_metamathqa_0920T03:38_merged 128 1 Qwen3-8B-Base 0.1 5e-7 10"
...
for experiment in "${experiments[@]}"; do
    ...
    top_k=${experiment_array[6]}
    echo "########################################################"
    echo "Running experiment: RePO - ${model_name} with lora rank ${lora_rank}"
    echo "########################################################"

    bash   ./bash_scripts/train_scripts/train_RePO_sweep_topk.sh \
        --pretrain $model_name \
        --save_path ./checkpoint/RePO_Qwen/RePO_${model_name_short}_lora_${lora_rank}_alpha_${alpha}_lr_${learning_rate}_topk_${top_k}_metamathqa \
        ...
        --use_wandb True \
        --input_template None \
        --wandb_run_name "RePO_${model_name_short}_lora_${lora_rank}_alpha_${alpha}_lr_${learning_rate}_topk_${top_k}_metamathqa" \
        --save_merged \
        --top_k $top_k
