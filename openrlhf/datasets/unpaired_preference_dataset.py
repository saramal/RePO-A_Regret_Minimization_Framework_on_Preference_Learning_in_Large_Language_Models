...
def preprocess_data(
    data, input_template=None, input_key=None, output_key=None, label_key=None, aRePOy_chat_template=None
):
    """
    ...
    if aRePOy_chat_template:
        if output_key:
            prompt = aRePOy_chat_template(data[input_key], tokenize=False, add_generation_prompt=True)
            response = aRePOy_chat_template(data[input_key] + data[output_key], tokenize=False)[len(prompt) :]
        else:
...
class UnpairedPreferenceDataset(Dataset):
    ...
    def process_data(self, data):
        prompt, response, label = preprocess_data(
            data, self.input_template, self.input_key, self.output_key, self.label_key, self.aRePOy_chat_template
        )
