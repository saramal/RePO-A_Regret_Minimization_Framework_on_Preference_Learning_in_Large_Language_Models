...
from openrlhf.datasets import SFTDataset, EvalDataset
from openrlhf.models import Actor
from openrlhf.trainer import SFTTrainer
from openrlhf.utils import blending_datasets, get_strategy, get_tokenizer
from openrlhf.utils import extract_last_answer, match_with_answer_labels_v2, extract_all_answers, match_with_answer_labels_v3
...
def eval(args):
    ...
    for benchmark_file in path.glob("*with_template.jsonl"):
        ...
        strategy.print(f"Loaded {len(eval_data)} samples from {benchmark_file} for evaluation.")
        # prepare dataset
        ...
        evaluate(model, tokenizer, strategy, eval_dataloader, benchmark_name)
    


def evaluate(model, tokenizer, strategy, eval_dataloader, benchmark_name: str):
    times = 0
    acc = 0
    model.eval()
