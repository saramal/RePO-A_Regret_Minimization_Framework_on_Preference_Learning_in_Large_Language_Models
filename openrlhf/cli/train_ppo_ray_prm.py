...
def reward_fn(rewards: list[Union[torch.Tensor, list[torch.Tensor]]]) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    prm_rewards = [reward for reward in rewards]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pretrain_data_probs",
        type=str,
        # default="1.0", #TODO: BEFORE
        default=None,  #TODO: AFTER
        help="sampling probs for datasets",
    )
    ...
    # split train dataset for rollout
    parser.add_argument("--split_train_dataset_ratio", type=float, default=None)
    ...
    # QWEN models use both "¿" and " ¿", so the version with a space should be specified.
    if args.placeholder_token:  
        args.placeholder_token = " " + args.placeholder_token
        args.prm_step_separator = args.placeholder_token
