...
def main(args):
    ...
    # RePO_data_ids_to_exclude=[]
    # with open(args.RePO_metadata, 'r') as f:
    #     for line in f:
    #         data = json.loads(line.strip())
    #         data_id = data["data_id"]
    #         RePO_data_ids_to_exclude.append(data_id)
    ...
    for data_id, prompt, label in query_dataset:    
        # if data_id in RePO_data_ids_to_exclude:
        #     continue
        data_ids.append(data_id)
        prompts.append(prompt)
        labels.append(label)
    strategy.print(f"Prepared generation dataset with {len(data_ids)} samples.")
    # batch size
    BATCH_SIZE = 10000
    ...
    for batch_idx in range(0, len(prompts), BATCH_SIZE):
        ...
        for data_id, query, output, answer in zip(batch_data_ids, batch_queries, outputs, batch_labels):
            query_result = {"prompt_id": data_id, "prompt": query, "responses": []}
            ...
            for resp in output.outputs:  # n responses
                ...
                # if resp.logprobs or args.n_logprobs is not None:
                ...
                # else:
                    ...
                    # # acc_per_query += is_correct
                    # if is_correct:
                    #     batch_results.append({"data_id": data_id, "query": query, "response": response_text})
