...
def compute_reward(
    ...
    if reward_clip_range:
        if isinstance(r, torch.Tensor):
            logger.info(f"LINE 69 : ORM reward_BEFORE: {r}")
            r = r.clamp(min=reward_clip_range[0], max=reward_clip_range[1])
            logger.info(f"LINE 71 : ORM reward_AFTER: {r}")
        elif isinstance(r, list):
            logger.info(f"LINE 72 : PRM reward_BEFORE: {r}")
            r = [r_.clamp(min=reward_clip_range[0], max=reward_clip_range[1]) for r_ in r]
            logger.info(f"LINE 74 : PRM reward_AFTER: {r}")
        else:
            raise ValueError("Unsupported type for r")
            
    logger.info(f"COMPUTE REWARD")
    ...
    if action_mask is not None:
        ...
        try:
            ...
            logger.info(f"LINE 103: reward_AFTER: {reward}")
        except:
            logger.info(f"kl_seg : {kl[0].shape}")
            logger.info(f"r_ : {r[0].shape}")
            reward = [r_ - kl_coef * kl_seg for kl_seg, r_ in zip(kl, r)]
            logger.info(f"LINE 107: reward_AFTER: {reward}")
    return reward
...
def reset_position_ids(attention_mask):
    logger.info(f"[DEBUG] LINE 133 : attention_mask: {attention_mask}")
