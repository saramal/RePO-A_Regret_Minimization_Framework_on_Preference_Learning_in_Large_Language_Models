...
from openrlhf.models import LogExpLoss, PairWiseLoss, RePO_Loss_deterministic, GPTLMLoss, RePO_Unvalanced_Loss
...
class RePOTrainerDeterministic(ABC):
    """
    Trainer for training a RePO model for deterministic generation.
    ...
    def __init__(
        ...
        self.eval_dataloader = eval_dataloader if eval_dataloader is not None else []
        ...
        if loss == "RePO":
            self.loss_fn = RePO_Loss_deterministic(self.cpl_lambda, self.ref_coef, self.alpha)          
        else:
            raise ValueError(f"Unknown loss type: {loss}. Supported types are 'RePO'.")
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        if self.strategy.is_rank_0():
            self.strategy.print(
                f"[RePOTrainer] max_global_step = {self.max_global_step}, "
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for local_step, data in enumerate(self.train_dataloader):
                ...
                if not self.packing_samples:
                    ...
                    if print_examples:
                        if self.strategy.is_rank_0():
                            ...
                            prompt_text = self.tokenizer.decode(data["prompt_ids"][0][0], skip_special_tokens=False)
                            chosen_text = self.tokenizer.decode(data["chosen_ids"][0][0], skip_special_tokens=False)
                            reject_text = self.tokenizer.decode(data["reject_ids"][0][0], skip_special_tokens=False)
                    ...
                    c_label_mask = data["chosen_label_masks"].squeeze(1).to(torch.cuda.current_device())
                    ...
                    r_label_mask = data["rejected_label_masks"].squeeze(1).to(torch.cuda.current_device())
                    ...
                    RePO_forward_output = self.concatenated_forward(
                            self.model, chosen_ids, c_mask, c_label_mask, reject_ids, r_mask, r_label_mask,  prompt_id_lens
                        )
                else:
                        ...
                        if self.ref_model:
                            with torch.no_grad():
                                ref_RePO_forward_output = self.concatenated_forward(
                                    self.ref_model, chosen_ids, c_mask, c_label_mask, reject_ids, r_mask, r_label_mask,  prompt_id_lens
                                )
                        else:
                            # treat model with disabled_adpater as ref_model
                            ref_RePO_forward_output = self.ref_concatenated_forward(
                                chosen_ids, c_mask, c_label_mask, reject_ids, r_mask, r_label_mask, prompt_id_lens
                            )
                    else:
                        ref_RePO_forward_output = self.ref_cached_forward(
                            ref_RePO_precalculated["ref_chosen_target_model_logprobs"], c_mask,
                            ref_RePO_precalculated["ref_reject_target_model_logprobs"], r_mask,
                        )
                ...
                logs_dict = {
                    ...
                    "regret_score": chosen_target_regret - reject_target_regret,
                ...
                pbar_dict = self.strategy.all_reduce(pbar_dict)
                step_bar.set_postfix(pbar_dict)
                step_bar.update()
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, c_label_mask, reject_ids, r_mask, r_label_mask, prompt_id_lens):
        """Run the given model on the given batch of inputs, concatenating the chosen and rejected inputs together.

        We do this to avoid doing two forward passes, because it's faster for FSDP.
        """
        input_ids, att_masks = self.concatenated_inputs(chosen_ids, c_mask, reject_ids, r_mask)
        _, label_masks = self.concatenated_inputs(chosen_ids, c_label_mask, reject_ids, r_label_mask)
        ...
        assert (chosen_ids.shape[0] == c_label_mask.shape[0])
        assert (reject_ids.shape[0] == r_label_mask.shape[0])
    ...
    def ref_concatenated_forward(self, chosen_ids, c_mask, c_label_mask, reject_ids, r_mask, r_label_mask, prompt_id_lens) -> dict:
        ...
        with torch.no_grad(), compte_ref_context_manager:
            if self.ref_model is None:
                with self.null_ref_context():
                    ref_model_output = self.concatenated_forward(self.model, chosen_ids, c_mask, c_label_mask, reject_ids, r_mask, r_label_mask, prompt_id_lens)
            else:
                ref_model_output = self.concatenated_forward(self.ref_model, chosen_ids, c_mask, c_label_mask, reject_ids, r_mask, r_label_mask, prompt_id_lens)
