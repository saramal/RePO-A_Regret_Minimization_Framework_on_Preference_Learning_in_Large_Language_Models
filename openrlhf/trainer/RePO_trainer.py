...
class RePOTrainer(ABC):
    ...
    def __init__(
        ...
        if loss == "RePO":
            self.loss_fn = RePO_Loss(self.cpl_lambda, self.ref_coef, self.alpha)          
        else:
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for local_step, data in enumerate(self.train_dataloader):
                ...
                if self.sft_loss and self.sft_loss_coef > 0:
                    labels = torch.where(
                    c_mask.bool(),
                    chosen_ids,
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        ...
        if save_by_ratio or save_by_step:
            ...
            if self.strategy.is_rank_0():
                self.strategy.print(
                    f"[RePOTrainer] Saving checkpoint at global_step={global_step} "
                    f"(ratio-based={save_by_ratio}, step-based={save_by_step})"
                )
            if not self.disable_ds_ckpt and save_by_step:
                self.strategy.save_ckpt(
                    self.model.model, args.ckpt_path, tag, args.max_ckpt_num, args.max_ckpt_mem, client_states
                )
            if self.save_hf_ckpt and save_by_step:
            ...
            for data in eval_dataloader:
                if not self.packing_samples:
                    if not self.strategy.args.disable_eval_loss:
                        ...
                        if self.sft_loss or self.sft_loss_coef > 0:  
                            labels = torch.where(
                            c_mask.bool(),
                            chosen_ids,
                            self.sft_loss_fn.IGNORE_INDEX,
