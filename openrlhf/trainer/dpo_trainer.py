...
class DPOTrainer(ABC):
                ...
                if not self.packing_samples:
                    ...
                    if self.ref_model:
                        ...
                        # treat model with disabled_adpater as ref_model
                            reference_chosen_logps, reference_rejected_logps, _, _ = self.ref_concatenated_forward(
                                    self.model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens
                                )
                else:
    ...
    def ref_concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens):
        ...
        with torch.no_grad(), compte_ref_context_manager:
            if self.ref_model is None:
                with self.null_ref_context():
                    ref_model_output = self.concatenated_forward(self.model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens)
            else:
                ref_model_output = self.concatenated_forward(self.ref_model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens)
