...
SAVE_PATH="${SAVE_PATH:-checkpoint/Qwen3-4B-metamathqa-RePO}"
...
deepspeed --master_port "$MASTER_PORT" --module openrlhf.cli.train_RePO \
  ...
  --chosen_logprob_key "${CHOSEN_LOGPROB_KEY:-chosen_logprob_with_token}" \
  --rejected_logprob_key "${REJECTED_LOGPROB_KEY:-rejected_logprob_with_token}" \
  --use_fast_dataset \
  --flash_attn \
  ...
  --sft_loss \
  --cpl_lambda "${CPL_LAMBDA:-1}" \
  --alpha "${ALPHA:-0.1}" \
  --sft_loss_coef "${SFT_LOSS_COEF:-1.0}" \
  --wandb_run_name "${WANDB_RUN_NAME:-RePO_public}" \
  "${EXTRA_FLAGS[@]}" \
  "$@"
